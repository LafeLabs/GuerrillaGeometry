...
void draw(){
  background(255);
  doTheThing(0300);
  currentGlyph = commandString2glyph(currentManuscriptFigures[4]);  
  drawCurrentGlyph();
  doTheThing(0300);
  x = 0;
  y = 40;
  ...
  x = 10;
  y = 400;
  currentGlyph = textString2glyph(currentManuscript[4]);
  side = 15;
  drawCurrentGlyph();
  exit();
}
...
void spellCurrentGlyph(){
  ...
  if(cursorOn){
    ...
    for(int index = cursorIndex;index < currentGlyph.length-1;index++){
     ...
     for(int index = 0;index < currentGlyph.length-1;index++){
       if(currentGlyph[index] >= 0300 && currentGlyph[index] <= 0377){
         drawGlyph(commandString2glyph(glyphArray0300s[currentGlyph[index] - 0300]));
       }
    }  
